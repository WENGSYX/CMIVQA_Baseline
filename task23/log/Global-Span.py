...
def collate_fn(data):
    ...
    vfeats_mask = torch.tensor([[1]*vfl+[0]*(768-vfl) for vfl in vfeat_lens])
...
def train_model(model,train_loader):  # 训练一个epoch
    ...
    for step, (input_ids, attention_mask, token_types,ious,vfeats,vfeats_mask) in enumerate(tk):
        ...
        loss = output['IOUloss']*0+output['CEloss']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--loss_hyperparameter", default=0., type=float)
    ...
    visual = load_video_features(os.path.join('data', 'features', 'I3D'), 768)
